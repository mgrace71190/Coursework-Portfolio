...
def index():
    ...
    rows = cash = db.execute("SELECT cash FROM users WHERE id=:user_id", user_id=session["user_id"])
    cash = rows[0]["cash"]
    grand_total += cash
...
def login():
    ...
    if request.method == "POST":
        ...
        # make sure UN and PW was submitted
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            return apology("issue with username duplicate", 403)
        if prim_key is None:
            return apology("issue with registration", 403)
